...
var popWin = {
    ...
    showWin: function(width, height, title, src) {
        ...
        var inntHtml = '';
        inntHtml += '<div id="mask" style="width:100%; height:100%; position:fixed; top:0; left:0; z-inde:22;background:#cccccc; filter:alpha(opacity=50); -moz-opacity:0.5; -khtml-opacity: 0.5; opacity:0.5;"></div>'
        inntHtml += '<div id="maskTop" style="visibility: hidden; width: ' + width + 'px; height: ' + height + 'px; border: #999999 1px solid; background: #fff; color: #333; position: fixed; top: 50%; left: 50%; margin-left: -' + marginLeft + 'px; margin-top: -' + marginTop + 'px; z-index: 23; filter: progid:DXImageTransform.Microsoft.Shadow(color=#909090,direction=120,strength=4); -moz-box-shadow: 2px 2px 10px #909090; -webkit-box-shadow: 2px 2px 10px #909090; box-shadow: 2px 2px 10px #909090;">'
        ...
        inntHtml += '</div>'
        inntHtml += '<iframe id="iframe"  width="' + width + '" height="' + iframeHeight + '" frameborder="0" scrolling="' + this.scrolling + '" src="' + src + '"></iframe></div>';
    ...
    autoCenter: function() {
        ...
        $iframe.on('load', function(evt) {
            var $panel = $(evt.target.contentWindow.document).find('#popup-panel'),
                width = $panel.width(),
                height = $panel.height(),
                left = width / 2,
                top = height / 2;
            ...
            $panel.on('click', '.btn-close', function(evt) {
